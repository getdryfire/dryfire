...
```sh
agentcheck run            # every suite listed in ../agentcheck.yaml
agentcheck run evals/hello.eval.yaml   # just one
agentcheck validate       # parse + check specs, no model calls
agentcheck trace hello_weather::reports_the_weather   # one case, turn by turn
```
...
- **`refund_agent.eval.yaml`** — real. It calls Anthropic and needs
  `ANTHROPIC_API_KEY`; without the key `agentcheck run` skips it (not a failure).
