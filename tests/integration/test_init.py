...
def test_init_then_run_is_green_with_no_key_and_no_network(
    ...
    # `dryfire run` — the command init told us to run — with cwd in the project.
...
def test_init_refuses_a_non_empty_dir_and_lists_conflicts(
    ...
    (tmp_path / "dryfire.yaml").write_text("PRECIOUS", encoding="utf-8")

    result = _init(tmp_path, monkeypatch)

    assert result.exit_code == 2
    assert "dryfire.yaml" in result.output
    assert (tmp_path / "dryfire.yaml").read_text(encoding="utf-8") == "PRECIOUS"
...
@pytest.mark.parametrize(
    "rel", ["dryfire.yaml", "evals/hello.eval.yaml", "evals/refund_agent.eval.yaml"]
)
