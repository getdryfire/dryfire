...
`dryfire init` lays down a runnable project — `dryfire.yaml`, the two
example suites, a shared schema, and a README — by copying
`dryfire/scaffold/template/**` verbatim. The template ships in the wheel
...
def _template_root() -> Traversable:
    return files("dryfire").joinpath("scaffold/template")
